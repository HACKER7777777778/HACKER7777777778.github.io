    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            ...
            if (contextOptions) {
                ...
                const imageUrls = [
                    { url: 'https://i.ibb.co/6HbYRpT/Captura-de-tela-2024-08-29-210805.png', cor: 'vermelha' },
                    { url: 'https://i.ibb.co/RDS5bK3/Captura-de-tela-2024-09-01-014104.png', cor: 'preta' },
                    { url: 'https://i.ibb.co/X2KPtR9/Captura-de-tela-2024-09-01-013952.png', cor: 'branca' }
                ];
...
// Função para ativar/desativar modo automático por comando de voz
function iniciarReconhecimentoVoz() {
    ...
    reconhecimento.onresult = function(evento) {
        ...
        // Ativar ou desativar o modo automático com voz
        if (comando.includes("ativar modo automático")) {
            if (!modoAutomaticoAtivado) {
                toggleModoAutomatico();
                falar("Modo automático ativado.");
            } else {
                falar("O modo automático já está ativado.");
            }
        } else if (comando.includes("desativar modo automático")) {
            if (modoAutomaticoAtivado) {
                toggleModoAutomatico();
                falar("Modo automático desativado.");
            } else {
                falar("O modo automático já está desativado.");
            }
        } else {
    ...
    reconhecimento.onerror = function(evento) {
        console.log("Erro no reconhecimento de voz:", evento.error);
        falar("Houve um erro ao tentar me escutar.");
    };
}

// Adiciona um botão para ativar a escuta
document.body.innerHTML += `<button onclick="iniciarReconhecimentoVoz()" style="position: fixed; bottom: 20px; right: 20px; padding: 10px 20px; background: blue; color: white; border: none; border-radius: 5px; cursor: pointer;">🎤 Falar</button>`;
